...
%description -l pl
Ten pakiet zawiera implementacjÍ agenta transportu wiadomo∂ci
internetowych o nazwie ZMailer. ZMailer przeznaczony jest dla bramek,
serwerÛw poczty lub innych ∂rodowisk wymagaj±cych niezwyk≥ych
moøliwo∂ci od mailera. Motywacj± dla ZMailera by≥y problemy z
Sendmailem w trudnych sytuacjach. ZMailer jest jednym z tych mailerÛw,
ktÛre potrafi± daÊ sobie radÍ z ogromn± ilo∂ci± poczty. Ponadto
ZMailer jest bardziej wydajny od innych mailerÛw w≥±czaj±c w to
qmaila. Kolejn± zalet± jest wsparcie dla protoko≥u IPv6, WHOSON, SSL
oraz TLS.

%package devel
Summary:	Static library and header file for zmailer
Summary(pl):	Plik nag≥Ûwkowy i biblioteka statyczna dla zmailera
...
To jest pakiet dla developerÛw. Zawiera plik nag≥Ûwkowy i bibliotekÍ
statyczn± ZMailera.
...
if [ ! -L /etc/mail/db/aliases ]; then
	if [ -f /etc/mail/aliases ]; then
		...
		rm -f /etc/mail/db/aliases || echo "Dziwnie pusto w (Strange nothing at) /etc/mail/db/aliases. Ale nie martw siÍ ... (But don't worry..)"
	else
